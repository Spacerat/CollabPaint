...
function rstr_hmac_md5(c,g){var a=rstr2binl(c);a.length>16&&(a=binl_md5(a,c.length*8));for(var b=Array(16),d=Array(16),e=0;e<16;e++)b[e]=a[e]^909522486,d[e]=a[e]^1549556828;a=binl_md5(b.concat(rstr2binl(g)),512+g.length*8);return binl2rstr(binl_md5(d.concat(a),640))}function rstr2hex(c){for(var g=hexcase?"0123456789ABCDEF":"0123456789abcdef",a="",b,d=0;d<c.length;d++)b=c.charCodeAt(d),a+=g.charAt(b>>>4&15)+g.charAt(b&15);return a}
...
function rstr2any(c,g){var a=g.length,b,d,e,f,h,i=Array(Math.ceil(c.length/2));for(b=0;b<i.length;b++)i[b]=c.charCodeAt(b*2)<<8|c.charCodeAt(b*2+1);var k=Math.ceil(c.length*8/(Math.log(g.length)/Math.log(2))),j=Array(k);for(d=0;d<k;d++){h=[];for(b=f=0;b<i.length;b++)if(f=(f<<16)+i[b],e=Math.floor(f/a),f-=e*a,h.length>0||e>0)h[h.length]=e;j[d]=f;i=h}a="";for(b=j.length-1;b>=0;b--)a+=g.charAt(j[b]);return a}
function str2rstr_utf8(c){for(var g="",a=-1,b,d;++a<c.length;)b=c.charCodeAt(a),d=a+1<c.length?c.charCodeAt(a+1):0,55296<=b&&b<=56319&&56320<=d&&d<=57343&&(b=65536+((b&1023)<<10)+(d&1023),a++),b<=127?g+=String.fromCharCode(b):b<=2047?g+=String.fromCharCode(192|b>>>6&31,128|b&63):b<=65535?g+=String.fromCharCode(224|b>>>12&15,128|b>>>6&63,128|b&63):b<=2097151&&(g+=String.fromCharCode(240|b>>>18&7,128|b>>>12&63,128|b>>>6&63,128|b&63));return g}
